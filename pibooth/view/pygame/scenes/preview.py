...
class PreviewScene(PygameScene):
    ...
    def __init__(self):
        ...
        # self.delay_counter = TextSprite(self,'5')
    ...
    def resize(self, size):
        ...
        self.image.set_rect(self.rect.width // 8, 6,
                            self.rect.width * 6 // 8, self.rect.height * 7 // 8)

        # Left image
        height = self.rect.height // 4
        size = (height, height)
        self.left_image.set_rect(0, self.rect.bottom - size[1], size[0] * 1.5, size[1])

        # Right image
        self.right_image.set_rect(self.rect.right - size[0] - 0, self.rect.bottom - size[1], size[0], size[1])
        ...
        # # Delay counter
        # self.delay_counter.set_rect(self.rect.right - size[0], self.rect.height // 3, size[0], size[1])
